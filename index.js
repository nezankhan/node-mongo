...
MongoClient.connect(url, { useUnifiedTopology: true }).then(client => {
    ...
    //assert.strictEqual(err, null); //we are check the first value "err" with the second value in this case "null" if err is equal to null then program will continue otherwise assert will fail and application will fail and error will be printed on console.

    console.log('Connected correctly to server');

    const db = client.db(dbname); //this will connect to db on mongo database

    /*Old Method using call back hell
